  ...
  '''
    ...
    def post_question():
        ...
        if 'searchTerm' in body.keys():
            return search_question(request, new_search)

        for key in ['question', 'answer', 'difficulty', 'category']:
            if key not in body.keys() or body[key] is None or body[key] == '':
                return abort(422)
    ...
    def search_question(request, search_term):
        try:
